      ...
      <div className={`flex flex-col h-full ${isCollapsed ? 'hidden' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            <div className="space-y-2">
                ...
                <div className="flex gap-2">
                    <select value={sortConfig.column} onChange={(e) => { setSortConfig(prev => ({ ...prev, column: e.target.value })); setActivePresetName(''); }} className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-sm cursor-pointer outline-none focus:border-green-500 transition-colors" title="Choose a column to sort the table by">
            ...
            <section className="space-y-4">
                <button onClick={() => setIsNameExpanded(!isNameExpanded)} className="w-full flex justify-between items-center text-xs font-bold text-slate-400 uppercase tracking-wider cursor-pointer px-2 py-2 rounded bg-slate-800/30 hover:bg-slate-800 transition-colors" title="Filter domains by name length, text content, prefixes, suffixes, hyphens, and numbers">
                ...
                {isNameExpanded && (
                    <div className="space-y-4">
                        <div className="space-y-1"><label className="text-xs text-slate-400">Length</label><div className="flex gap-2"><input type="number" placeholder="Min" value={filters.minLength} onChange={(e) => updateFilter('minLength', e.target.value)} className="w-1/2 bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm outline-none focus:border-slate-500" title="Minimum domain name length (excluding TLD)"/><input type="number" placeholder="Max" value={filters.maxLength} onChange={(e) => updateFilter('maxLength', e.target.value)} className="w-1/2 bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm outline-none focus:border-slate-500" title="Maximum domain name length (excluding TLD)"/></div></div>
                        ...
                        <div className="flex gap-2"><input type="text" value={filters.startsWith} onChange={(e) => updateFilter('startsWith', e.target.value)} className="w-1/2 bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-sm outline-none focus:border-green-500" placeholder="Starts With" title="Show only domains that begin with this text"/><input type="text" value={filters.endsWith} onChange={(e) => updateFilter('endsWith', e.target.value)} className="w-1/2 bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-sm outline-none focus:border-green-500" placeholder="Ends With" title="Show only domains that end with this text (before the TLD)"/></div>
                        <div className="flex gap-2 pt-1"><div className="w-1/2 space-y-1"><label className="text-xs text-slate-400">Hyphens</label><select value={filters.hyphenSetting} onChange={(e) => updateFilter('hyphenSetting', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm outline-none cursor-pointer hover:bg-slate-750 transition-colors" title="Filter by number of hyphens in domain name"><option value="any">Any</option><option value="none">None</option><option value="max1">Max 1</option><option value="max2">Max 2</option></select></div><div className="w-1/2 space-y-1"><label className="text-xs text-slate-400">Numbers</label><select value={filters.numberSetting} onChange={(e) => updateFilter('numberSetting', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm outline-none cursor-pointer hover:bg-slate-750 transition-colors" title="Filter by digits in domain name"><option value="any">Any</option><option value="none">None</option><option value="max1">Max 1</option><option value="max2">Max 2</option><option value="only">Only</option></select></div></div>
                    </div>
                )}
            </section>
            <section className="space-y-4">
            ...
            <section className="space-y-2">
                <button onClick={() => setIsColumnsExpanded(!isColumnsExpanded)} className="w-full flex justify-between items-center text-xs font-bold text-slate-400 uppercase tracking-wider cursor-pointer px-2 py-2 rounded bg-slate-800/30 hover:bg-slate-800 transition-colors" title="Show or hide table columns to customize your view">
                    <span className="flex items-center gap-2">
                        Toggle Columns
            ...
            <section className="space-y-2">
                <button onClick={() => setIsAdvancedExpanded(!isAdvancedExpanded)} className="w-full flex justify-between items-center text-xs font-bold text-slate-400 uppercase tracking-wider cursor-pointer px-2 py-2 rounded bg-slate-800/30 hover:bg-slate-800 transition-colors" title="Enable presets, heatmap visualization, and custom pattern matching">
                    <span className="flex items-center gap-2">
                        Advanced
        ...
        <div className="p-4 bg-slate-800/50 border-t border-slate-800 space-y-3 shadow-lg">
             ...
             <div className="flex gap-2">
                <button onClick={copyVisible} disabled={!!copyFeedback || visibleCount === 0} className={`flex-1 font-bold py-2 rounded transition-all shadow-lg text-sm ${visibleCount === 0 ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : copyFeedback ? 'bg-green-700 text-white cursor-pointer' : 'bg-green-600 hover:bg-green-500 text-white active:scale-95 cursor-pointer'}`} title={visibleCount === 0 ? 'No domains to copy' : 'Copy all visible domain names to clipboard'}>{copyFeedback || 'Copy Visible Domains'}</button>
                ...
                <section className="border-t border-slate-800 pt-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="pt-2 flex flex-col gap-2">
                            <a href="https://github.com/roie/domain-powertools/issues" target="_blank" rel="noopener noreferrer" className="text-xs text-green-400 hover:text-green-300 transition-colors flex items-center gap-1">
